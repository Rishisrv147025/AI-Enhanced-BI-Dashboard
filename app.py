...
from chart_generator import train_model
from query_processing import execute_query
from utils import get_operations_list,chart_list
...
if uploaded_file:
    ...
    if data is not None:
        ...
        # Chart Generator
        st.subheader('Chart Generator on Data')
        chart_query = st.text_input("Enter Query:")
        if chart_query:
            model = train_model(chart_query, data)
            st.write("Query Result:", model)
